...
-- Find a low-health opponent to attack
function findTarget()
    local target = nil
...
-- Attack the target using 30% of energy
function attackTarget()
    local target = findTarget()

    if target then
        local attackEnergy = LatestGameState.Players[ao.id].energy * 0.3
        print(colors.red .. "Attacking target from above with energy: " .. attackEnergy .. colors.reset)
        ao.send({ Target = Game, Action = "PlayerAttack", Player = ao.id, AttackEnergy = tostring(attackEnergy) }) -- Attack with 30% of energy
        evadeAndAscend() -- Immediately evade and ascend after attacking
        return true
    end
...
-- Evade and ascend to avoid counterattacks
function evadeAndAscend()
    local self = LatestGameState.Players[ao.id]
    print(colors.green .. "Evading and ascending." .. colors.reset)
    -- Simulate ascending by moving up on the y-axis (or any appropriate direction in your game)
    ao.send({ Target = Game, Action = "Move", X = self.x, Y = self.y - 1 })
    InAction = false -- Reset InAction after moving
end

-- Patrol the arena to scout for opponents
function patrol()
    local self = LatestGameState.Players[ao.id]
    ...
    print(colors.gray .. "Patrolling the arena." .. colors.reset)
    InAction = false -- Reset InAction after moving
end
...
function decideNextAction()
    if not attackTarget() then
        -- If no target found, patrol the arena
        patrol()
    end
end
